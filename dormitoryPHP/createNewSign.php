<?php
    ...
    $latitude=$_POST['latitude'];//APP post过来的纬度
    $longitude=$_POST['longitude'];//APP post过来的纬度
    $sql=mysqli_query($conn,"SELECT COUNT(*) as 'totalnums' FROM userinfo WHERE belong LIKE '$govern'");
    $totalnums=0;
    if($result=mysqli_fetch_assoc($sql)){
        $totalnums=$result['totalnums'];
    }
    $result=mysqli_query($conn,"INSERT INTO signrecord (Rtime,houseparentID,title,govern,totalnums,latitude,longitude) VALUES('$Rtime','$houseparentID','$title','$govern','$totalnums','$latitude','$longitude')");
     if($result){
       $back['status']="13";
